...
export class CreatePageComponent implements OnInit {
  ...
  constructor(private vouch: VoucherService, private pag: PagosService, private vent: VentasService) {

  }

  static createFormGropu(){
  ...
  cuandoelijo(ventapago) {
    if (ventapago.mpnombre === 'TARJETA') {
      this.devolver  = null;
      this.ejerciciodevolucion = 0;
      document.getElementById('devolucion').hidden = true;
    }

    if (ventapago.mpnombre === 'EFECTIVO') {
  ...
  ngOnInit() {
    ...
    this.pag.mostrarmediodepago().subscribe(fmedio => { this.mediopago = fmedio; });

    this.vent.mostrarventas().subscribe(esto => { this.estotengo = esto; });
     this.pag.mostrarpagos().subscribe( tengo => {this.elpago = tengo; } );
  }
